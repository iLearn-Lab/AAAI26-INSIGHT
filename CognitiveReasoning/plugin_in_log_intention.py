...
from my_rewards_new_intention import ActionIntentReward, _clean_intention
from my_rewards_new_intention import list_damerau_distance
...
class ActionIntentContLin(ORM):
    ...
    def __call__(
        ...
        **kwargs
        ...
        scores = []
        gt_list = gt_answer if gt_answer is not None else solution
        for idx, (pred_full, msg) in enumerate(zip(completions, messages)):
            if gt_list is not None:
                gt_content = gt_list[idx]
            else:
                gt_content = self._extract_reference_assistant(msg)
            gt_actions = (gt_content.split('<answer>',1)[1]
                                  .split('</answer>',1)[0]) if '<answer>' in gt_content else ""
            gen_actions = (pred_full.split('<answer>',1)[1]
                                  .split('</answer>',1)[0]) if '<answer>' in pred_full else ""
            gt_pairs  = [t.lower().strip() for t in gt_actions.split(',') if self._valid_pair(t)]
            gen_pairs = [t.lower().strip() for t in gen_actions.split(',') if self._valid_pair(t)]
            raw = list_damerau_distance(gt_pairs[:self.max_len], gen_pairs[:self.max_len])
            raw += max(0, self.max_len - len(gen_pairs))
            s_lin = 1.0 - raw/self.max_len
            scores.append(max(0.0, s_lin))
        return scores
    ...
    def __call__(
        ...
        **kwargs
        ...
        for idx, (pred_full, msg) in enumerate(zip(completions, messages)):
            ...
            raw = list_damerau_distance(gt_pairs[:self.max_len], gen_pairs[:self.max_len])
            raw += max(0, self.max_len - len(gen_pairs))
            s_pow = (1.0 - raw/self.max_len)**7
            scores.append(max(0.0, s_pow))
        return scores

class ExternalIntentScoreORM(ORM):

    def __init__(self, *args, **kwargs):
        super().__init__()

        self.intent_scorer = ActionIntentReward(*args, **kwargs)

    def _extract_reference_assistant(self, msg: List[Dict]) -> str:

        return self.intent_scorer._extract_reference_assistant(msg)

    def __call__(
        self,
        completions: List[str],
        messages: List[List[Dict]],
        gt_intention: Optional[List[str]] = None,
        gt_answer: Optional[List[str]] = None,
        solution: Optional[List[str]] = None,
        **kwargs,
    ) -> List[float]:
        ...
        if gt_intention is not None:
            gt_ints_raw = gt_intention
        else:

            gt_text_container = gt_answer if gt_answer is not None else solution
            if gt_text_container is not None:
                gt_ints_raw = []
                for t in gt_text_container:
                    m = re.search(r'<intention>(.*?)</intention>', t, flags=re.DOTALL)
        ...
        gen_ints_raw = []
        for pred_full in completions:
            m = re.search(r'<intention>(.*?)</intention>', pred_full, flags=re.DOTALL)
            gen_ints_raw.append(m.group(1) if m else "")


        gt_ints = [_clean_intention(t) for t in gt_ints_raw]
        gen_ints = [_clean_intention(t) for t in gen_ints_raw]


        if not any(t.strip() for t in gt_ints) or not any(t.strip() for t in gen_ints):
            return [0.0] * B


        emb_all = self.intent_scorer._embed(gt_ints + gen_ints)
        gt_embs = emb_all[:B]
        gen_embs = emb_all[B:]
        sims = F.cosine_similarity(gt_embs, gen_embs, dim=1)


        scores = [self.intent_scorer._sim_to_score(float(s)) for s in sims]
        return scores

class ExternalIntentScoreRAW(ORM):

     def __init__(self, *args, **kwargs):
         super().__init__()

         from my_rewards_intention import ActionIntentReward
         self.intent_scorer = ActionIntentReward(*args, **kwargs)

     def _extract_reference_assistant(self, msg):
     ...
     def __call__(self,
         ...
         if gt_intention is not None:
             ...
             src = gt_answer if gt_answer is not None else solution
             if src is not None:
                 gt_ints_raw = []
                 for t in src:
                     m = re.search(r'<intention>(.*?)</intention>', t, flags=re.DOTALL)
                 ...
                 gt_ints_raw = [ self._extract_reference_assistant(msg)
                                 for msg in messages ]

         gen_ints_raw = []
         for pred in completions:
             m = re.search(r'<intention>(.*?)</intention>', pred, flags=re.DOTALL)
             gen_ints_raw.append(m.group(1) if m else "")


         from my_rewards_intention import _clean_intention
         gt_ints = [_clean_intention(t) for t in gt_ints_raw]
         gen_ints = [_clean_intention(t) for t in gen_ints_raw]
         ...
         import torch.nn.functional as F
         embs = self.intent_scorer._embed(gt_ints + gen_ints)
         gt_embs, gen_embs = embs[:B], embs[B:]
         sims = F.cosine_similarity(gt_embs, gen_embs, dim=1)
...
orms['external_action_intent'] = ActionIntentReward
orms['external_action_cont_lin'] = ActionIntentContLin
orms['external_action_cont_pow'] = ActionIntentContPow 
orms['external_intent_score'] = ExternalIntentScoreORM
orms['external_intent_score_raw'] = ExternalIntentScoreRAW
